...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto mt-8 px-4">
        ...
        <?php endif; ?>
        <?php if (isset($error) || session()->has('success')) : ?>
            <div class="bg-green-200 text-green-800 p-3 mb-4"><?= isset($error) ? $error : session('success') ?></div>
        <?php endif; ?>

        <button class="bg-green-500 mb-3 flex hover:bg-green-600 text-white py-2 px-4 rounded mt-4 ">
            <a href="<?= site_url('movimientos/create') ?>">
            ...
            <tbody>
                <?php foreach ($movimientos as $movimiento) : ?>
                    <tr class="dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <td class=" p-3"><?= $movimiento['id'] ?></td>
                        <td class=" p-3"><?= $movimiento['descripcion'] ?></td>
                        <td class=" p-3"><?= $movimiento['fecha'] ?></td>
                        <td class="flex p-3">
                            <a href="<?= site_url('movimientos/edit/' . $movimiento['id']) ?>" class="text-blue-500 hover:underline">
                            ...
                            <a href="<?= site_url('movimientos/delete/' . $movimiento['id']) ?>" class="text-red-500 hover:underline ml-2">
